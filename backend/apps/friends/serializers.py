...
class FriendshipSerializer(serializers.ModelSerializer):
    ...
    accepter = serializers.SlugRelatedField(
        queryset=User.objects.all(), slug_field='username')
    ...
    def create(self, validated_data):
        ...
        if Friendship.objects.filter(inviter=user_2, accepter=user_1).exists():
            # Проверка на статусы нужна!!
            Friendship.objects.filter(inviter=user_2, accepter=user_1).update(
            ...
            Friendship.objects.create(
                inviter=user_1, accepter=user_2, status=Friendship.Status.INVITED)
    ...
    def update(self, validated_data):
        ...
        if Friendship.objects.filter(inviter=user_1, accepter=user_2).exists():
            friendship = Friendship.objects.get(
                inviter=user_1, accepter=user_2)
            ...
            if friendship.status == Friendship.Status.FRIENDS:
                setattr(friendship, 'inviter', user_2)
                setattr(friendship, 'accepter', user_1)
                setattr(friendship, 'status', Friendship.Status.INVITED)
                friendship.save()
            ...
            friendship = Friendship.objects.get(
                inviter=user_2, accepter=user_1)
            if friendship.status == Friendship.Status.INVITED:
                friendship.delete()
                return Response({"detail": 'Заявка успешно отклонена'}, status=status.HTTP_200_OK)
            if friendship.status == Friendship.Status.FRIENDS:
                setattr(friendship, 'inviter', user_1)
                setattr(friendship, 'accepter', user_2)
                setattr(friendship, 'status', Friendship.Status.INVITED)
                friendship.save()
                return Response({"detail": "Пользователь успешно удален из друзей"}, status=status.HTTP_202_ACCEPTED)
        else:
